...
/**
 * Display an OES object as xml. @oesDevelopment In Development.
 */
 ...
 * @return string
 */
function oes_prepare_xml(array $args = []): string
{
    global $oes, $oes_post, $post_type;
    ...
    if(!$oes_post) {
        ...
        $oes_post = class_exists($post_type) ? new $post_type($postID) : new OES_Post($postID);
    }
    ...
    foreach (array_keys($oes->taxonomies) as $taxonomy)
        if ($taxonomyTerms = get_the_terms($oes_post->object_ID, $taxonomy))
