...
if (!class_exists('Container')) :
     ...
     * TODO @nextRelease : add field options (to filter content on info page) to config options and enhance (as for now: only select field)
     */
    class Container extends Page
    {
        ...
        public function __construct(string $key, array $containerArgs = [])
        {
            ...
            foreach ($subPages as $key => $page) {
                ...
                if (!$infoPageElements && $position === 1) {
                    ...
                    /* redirect first sub-menu. TODO @nextRelease : is there a better way to do this? */
        ...
        function html()
        {
                ...
                foreach ($subPages as $pageKey => $page)
                    if (isset($page['display_summary']) && $page['display_summary']) {
                        ...
                        if ($postType) {
                            ...
                            $postArray = [];
                            $args = wp_parse_args(['post_type' => $postType->name], $this->get_posts_defaults);
                                ...
                                if ($postArray) {
                                    ...
                                    if (has_filter('oes/menu_container_posts'))
                                        $postArray = apply_filters('oes/menu_container_posts',
                                            $postArray, $postType, $selectValue);
            ...
            if (has_filter('oes/page_container_content'))
                $pageContent = apply_filters('oes/page_container_content', $pageContent, $this->main_slug);
...
/* create menu pages */
add_action('oes/datamodel_registered', 'OES\Admin\initialize_container_pages');

/**
 * Create container pages
 */
function initialize_container_pages(){
    $oes = OES();
