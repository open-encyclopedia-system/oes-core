...
/**
 * Create a help tab on "OES" page.
 * @param $screen
 * @return void
 */
function toplevel_page_oes_settings_help_tabs($screen): void
{
    $screen->add_help_tab([
    ...
    $screen->add_help_tab([
        'id' => 'oes_general_configuration',
        'title' => 'Configuration',
        'content' => '<p>' .
            __('OES provides functionalities for custom WordPress post types and taxonomies that are part of an ' .
                'OES application plugin. OES post types can be configured in the backend or the editorial layer ' .
    ...
    $screen->set_help_sidebar('<p>' .
        sprintf(__('For more information please visit our %swebsite%s.', 'oes'),
...
/**
 * Create a help tab on "OES Index" page.
 * @param $screen
 * @return void
 */
function oes_page_oes_settings_index_help_tabs($screen): void
{
    $screen->add_help_tab([
...
/**
 * Create a help tab on "OES Language & Text" page.
 * @param $screen
 * @return void
 */
    ...
    $screen->add_help_tab([
        ...
        'content' => '<p>' .
            __('If you are using an OES theme you can define labels for the templates that will be rendered ' .
                ...
                '<b>Bilingualism</b>. Most of the labels are defined by your OES application plugin.', 'oes') .
            '</p><p>' .
...
/**
 * Create a help tab on "OES Lod" page.
 * @oesDevelopment Review, link to manual etc.
 * @param $screen
 * @return void
 */
function oes_page_oes_settings_lod_help_tabs($screen): void
{
    $screen->add_help_tab([
        'id' => 'oes_lod_help',
        'title' => 'LOD',
...
/**
 * Create a help tab on "OES Media" page.
 * @param $screen
 * @return void
 */
function oes_page_oes_settings_media_help_tabs($screen): void
{
    $screen->add_help_tab([
...
/**
 * Create a help tab on "OES Schema" page.
 * @param $screen
 * @return void
 */
...
/**
 * Create a help tab on "OES Search" page.
 * @param $screen
 * @return void
 */
function oes_page_oes_settings_search_help_tabs($screen): void
{
    $screen->add_help_tab([
...
/**
 * Create a help tab on "OES Help" page.
 * @param $screen
 * @return void
 */
function oes_page_oes_settings_theme_help_tabs($screen): void
{
    global $oes;
...
/**
 * Create a help tab on "OES Advanced" page.
 * @param $screen
 * @return void
 */
function oes_page_oes_settings_advanced_help_tabs($screen): void
{
    $screen->add_help_tab([
    ...
    $screen->add_help_tab([
        ...
        'content' => '<p>' .
            __('The OES feature Container allows you to organize admin menu items into a new top menu with sub ' .
                ...
                'inside your OES application plugin to be available for configuration, see user manual for more information.', 'oes') .
            '<ul>' .
...
function oes_tools_page_oes_tools_model_help_tabs($screen): void
{
    ...
    $screen->add_help_tab([
        ...
        <p>The OES <em>Data Model</em> feature allows you to configure the appearance and behavior of the objects defined in your OES application plugin.</p>
        ...
        <a href="https://developer.wordpress.org/reference/functions/register_taxonomy/" target="_blank">custom taxonomies</a>. Note that not all options are available through the OES interface — some advanced settings require editing the configuration directly in your application plugin.</p>
        ...
        <p>If no post types or taxonomies appear in the list below, check your application\'s data model configuration or review the OES Admin settings.</p>
    ')
    ]);
...
function toplevel_page_admin_manual_help_tabs($screen): void
{
    ...
    $screen->add_help_tab([
        'id' => 'oes_manual_help',
        'title' => 'Overview',
        'content' => '<p>' . __('These guidelines serves as a living resource curated by the application team — meaning: ' .
            ...
            '<li>' . __('Frequently Asked Questions (FAQs) — quick answers to common questions that arise during application work', 'oes') . '</li>' .
            '<li>' . __('Author’s Guide — practical guidance tailored to writing for this specific application', 'oes') . '</li>' .
            '<li>' . __('Styling Tips — application-specific visual patterns or formatting recommendations', 'oes') . '</li>' .
            '<li>' . __('Development Requests — a lightweight way to log content-related tasks or blockers that need developer support', 'oes') . '</li>' .
            '</ul>' .
            '<p>' .
            __('These guidelines are built for flexibility and collaboration. If something\'s missing or unclear, ' .
...
function toplevel_page_admin_oes_remarks_help_tabs($screen): void
{
    ...
    $screen->add_help_tab([
        ...
        'content' => '<p>OES Remarks provides an overview of all internal Comment fields across your OES objects ' .
            ...
            'encyclopedia application.</p>',
    ]);

    $screen->set_help_sidebar('<p><strong>See </strong></p><p><a href="https://manual.open-encyclopedia-system.org/">OES Manual</a></p>');
}

function oes_tools_page_oes_tools_import_help_tabs(): void
{
    $screen = get_current_screen();

    $screen->add_help_tab([
        'id'      => 'oes_import_overview',
        'title'   => __('Overview', 'oes'),
        'content' =>
            '<p>' . esc_html__('Upload a UTF-8 encoded CSV file to prepare import operations.', 'oes') . '</p>' .
            '<p>' . esc_html__('Valid operations are "insert" and "update". Update requires a valid object ID.', 'oes') . '</p>'
    ]);

    $screen->add_help_tab([
        'id'      => 'oes_import_format',
        'title'   => __('File Format', 'oes'),
        'content' =>
            '<p>' . esc_html__('The CSV file must follow the required field format.', 'oes') . '</p>' .
            '<p><a href="' . esc_url(admin_url('admin.php?page=oes_tools_export')) . '">' .
            esc_html__('Download template', 'oes') .
            '</a></p>' .
            '<p><a href="https://developer.wordpress.org/reference/classes/wp_post/" target="_blank">' .
            esc_html__('WP_Post reference', 'oes') .
            '</a></p>'
    ]);

    $screen->set_help_sidebar(
        '<p><strong>' . esc_html__('Import Notes', 'oes') . '</strong></p>' .
        '<p>' . esc_html__('Time limit: 10 minutes', 'oes') . '</p>' .
        '<p>' . esc_html__('UTF-8 encoding required', 'oes') . '</p>'
    );
}
