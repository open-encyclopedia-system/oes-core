...
class Factory extends Tool
{
        ...
        if (!empty($adminOnly)): echo $adminOnly;
            ...
            if (!$this->factory_mode):?>
                <span>
                    ...
                    <p class="description">
                        <?php _e('Overwrites the current OES data model using factory items.', 'oes'); ?>
                    </p>
                </span><?php
    ...
    {
        $factoryService = new \OES\Model\Factory_Service();

        if (isset($_POST['import_to_factory'])) {
            $factoryService->import_to_factory();
        } elseif (isset($_POST['import_factory'])) {
            $factoryService->import_from_factory();
        } elseif (isset($_POST['reset_factory'])) {
