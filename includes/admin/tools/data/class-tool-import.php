...
if (!class_exists('Import')) :
    ...
    class Import extends Tool
    {
        ...
        function greet()
        {
            ...
            $count = $wpdb->get_results("SELECT count(`operation_status`) AS count from $table WHERE `operation_status` <> 'ignored' OR `operation_status` <> 'success' OR `operation_status` <> 'error'");
        ...
        function parse(array $file): bool
        {
            ...
            if (sizeof($firstRow) < 2) {
                $this->admin_notices[] = [
                    'notice' => __('The file appears to be empty. Check the file and make sure that more than the first column is filled.', 'oes'),
                    'type' => 'error'
                ];
                return false;
            }
            ...
            $countRows = 0;
            $allRows = [];
            while (($fileRow = fgetcsv($handle, 0, ";")) !== FALSE && ($lastRow < 1 || $countRows < $lastRow + 1)) {
            ...
            foreach ($prepareOperations as $data) {
                ...
                if (isset($data['args']['object_args']) &&
                    ...
                    if ($success) $countSuccess++;
                        ...
                        'notice' => '<p>' . sprintf(__('Error while inserting operation for row %s.', 'oes'), $tempID) . '</p>',
                        'type' => 'error'
                    ];
                }
                ...
                if (isset($data['args']['meta']) && sizeof($data['args']['meta']) > 0)
                    foreach ($data['args']['meta'] as $key => $value) {
                        ...
                        if ($success) $countSuccess++;
                            ...
                            'notice' => sprintf('<p>' . __('Error while inserting operation for row %s.', 'oes') . '</p>',
                                $tempID),
                            'type' => 'error'
                        ];
                    }
            ...
            if ($countSuccess)
                $this->admin_notices[] = [
                    'notice' => sprintf(__('%s operation%s ha%s been inserted into the database. You can see the prepared operations %shere%s.', 'oes'),
                        $countSuccess,
                        ($countSuccess == 1 ? '' : 's'),
                        ($countSuccess == 1 ? 's' : 've'),
                    ...
                    foreach ($matchArray['fields'] as $tableKey => $match) {
                        if (is_array($match)) {
                            $fieldKey = $match['field_key'] ?? ($match['repeater_base'] ?
                                ($match['repeater_base'] . '_' . ($match['sequence'] ?? 0) . '_' . ($match['repeater_field_key'] ?? '')) :
                    ...
                    foreach ($returnValues as $subArray => $subArrayValues) {
                        foreach ($subArrayValues as $fieldKey => $fieldValue) {
                            ...
                            if (isset($tempStore[$fieldKey])) {
                                ...
                                } elseif (isset($tempStore[$fieldKey]['repeater']) && $tempStore[$fieldKey]['repeater'] &&
