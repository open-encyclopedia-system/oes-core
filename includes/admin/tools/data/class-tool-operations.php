...
if (!class_exists('Operations')) :
    ...
    class Operations extends Tool
    {
        ...
        function html(): void
        {
            ...
            $count = $wpdb->get_results("SELECT `operation_status`, `operation_type`, count(`operation_status`) AS count FROM $table GROUP BY `operation_status`, `operation_type`"); //@oesDevelopment WHERE `operation_status` <> 'ignored'
        ...
        function display_operation_submit_form()
        {
            ...
            <h2 class="nav-tab-wrapper"><?php
                ...
                $selectedOperation = $_GET['select'] ?? (array_key_first($this->count_array[$selectedType]['count_per_operation']) ?? 'unknown');
            ...
            if (isset($this->count_array[$selectedType]['count_per_operation']) &&
                ...
                foreach ($this->count_array[$selectedType]['count_per_operation'] as $operation => $count)
                    $operationsList[] = sprintf('<a href="%s" class="%s" data-type="%s">%s <span class="count">(%s)</span></a>',
            ...
            if (!empty($operationsList))
                echo '<div class="oes-operation-select"><ul class="subsubsub"><li>' . implode('</li> | <li>', $operationsList) . '</li></ul></div>';
        ...
        function greet()
        {
            ...
            _e('This tool allows you to view the available database operations and execute or delete them. ' .
                'You can view a list of all available operations and decide which operations will be executed below.', 'oes');
            echo '</p><p>';
            _e('You can import operations using the import tool.', 'oes');
            echo '</p>';
            echo '<a href="' . admin_url('admin.php?page=tools_import') . '" class="button button-secondary">' . __('Import Operations', 'oes') . '</a>';
            echo '</div>';
        }
        ...
        function admin_post_tool_action(): void
        {
            ...
            if (isset($_POST['operation-action'])) {
                ...
                if ($_POST['operation-action'] === 'drop') {
                    global $wpdb;
                    $wpdb->query('DROP TABLE ' . $wpdb->prefix . 'oes_operation');
                }
                elseif ($_POST['operation-action'] === 'empty') {
                    ...
                    if (count($operations) > 0)
                        foreach ($operations as $operation) {
                            ...
                            if ($_POST['operation-action'] === 'reset') {
                                ...
                                else $errorMessages[] = sprintf(__('There has been an error while trying to reset the status for operation %s.'), $operation->id);
                                $allOperations++;
                                ...
                                if (!in_array($op->operation_status, ['ignored', 'success', 'error'])) {
                                    ...
                                    else $errorMessages[] = sprintf(__('There has been an error while trying to execute operation %s.'), $operation->id);
                                    $allOperations++;
                                }
                                ...
                                if ($operation->operation_status === $statusKey) {
                                    ...
                                    else $errorMessages[] = sprintf(__('There has been an error while trying to delete operation %s.'), $operation->id);
                                    $allOperations++;
                                }
                            }
                        }
                    ...
                    if ($allOperations < 1) {
                        ...
                        $message .= sprintf(__(' There ha%s been %s error%s while trying to process operations. Error messages:%s', 'oes'),
                            ($countErrors == 1 ? 's' : 've'),
                            $countErrors,
                            ($countErrors == 1 ? '' : 's'),
