    ...
    foreach ($results as $row) {
        ...
        if ($isTimeout) {
            ...
            if (!isset($transients[$name])) {
                ...
                [$prefix, $key, $archiveClass, $languageKey] = array_pad(explode('-', $name), 4, null);

                $label =
                    $oes->theme_index_pages[$key]['label']['language0'] ??
                    $oes->taxonomies[$key]['label'] ??
                    $oes->post_types[$key]['label'] ??
                    __('Unknown Post Type', 'oes');

                $cleanName = '<b>' . $label . '</b> ';
                $cleanName .= '<i>' . ($archiveClass ?? __('Unknown Archive Class', 'oes')) . '</i> ';
                $cleanName .= $oes->languages[$languageKey ?? 'none']['label'] ?? __('Unknown Language', 'oes');
