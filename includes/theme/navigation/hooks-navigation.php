...
use OES\Navigation\Template_Redirect;
use function OES\ACF\oes_get_field;
use function OES\Multilingualism\get_language_switch;

add_action('admin_head-nav-menus.php', 'oes_navigation_admin_head_nav_menus');
add_action('init', 'oes_navigation_init');
add_action('template_redirect', 'oes_navigation_template_redirect');
add_filter('nav_menu_item_title', 'oes_navigation_modify_title', 10, 2);
add_filter('nav_menu_link_attributes', 'oes_navigation_modify_atts', 10, 2);
add_filter('nav_menu_css_class', 'oes_navigation_modify_css', 10, 2);


/**
 * Add OES menu meta box to theme menu page
 */
function oes_navigation_admin_head_nav_menus(): void
{
    add_meta_box(
        'oes_menu_items',
        __('OES Menu Items', 'oes'),
        'oes_navigation_render_menu_meta_box',
        'nav-menus',
        'side',
        'high',
        []);
 ...
 */
function oes_navigation_init(): void
{
        ...
        [
            ...
            'fields' => [
                ...
                [
                    ...
                    'type' => 'text',
                    'instructions' => 'Replace the label with an svg icon. Enter the svg file name including the relative path.',
                ]
            ],
            'location' => [[[
                'param' => 'nav_menu_item',
 ...
 * Redirect templates
 */
function oes_navigation_template_redirect(): void
{
    if (!is_admin()) {
        global $oes, $oes_template_redirect;
        $projectThemeClass = str_replace('-', '_', $oes->basename_project) . '_Template_Redirect';
        $oes_template_redirect = class_exists($projectThemeClass) ?
            new $projectThemeClass() :
            new Template_Redirect();
    }
}
 ...
 */
function oes_navigation_modify_css(array $classes, WP_Post $item): array
{
 ...
 */
function oes_navigation_modify_title(string $title, WP_Post $item): string
{
    ...
    elseif ($image = oes_get_field('field_replace_label_image', $item->ID)) {
        ...
        if (is_array($image) && isset($image['ID'])) {
            ...
            if (!empty($path) &&
                file_exists(WP_CONTENT_DIR . '/uploads/' . $path)) $html = oes_get_html_img(esc_url($image['url']), esc_attr($image['alt']));
        }

        if (empty($html))
            $html = '<span class="oes-nav-replace-with-image">' . ($image['alt'] ?: ($image['title'] ?: '')) . '</span>';
 ...
 */
function oes_navigation_modify_atts(array $atts, WP_Post $item): array
{
    ...
    if (property_exists($item, 'object') && $item->object === 'oes-menu-item-search') {
        $atts['id'] = 'oes-search';
    } elseif (property_exists($item, 'object') && $item->object === 'oes-menu-item-language-switch') {
        $languageSwitch = get_language_switch();
        $linkObject = $languageSwitch->item_link_object();
        if (isset($linkObject['link'])) $atts['href'] = $linkObject['link'];
    }
    return $atts;
}
 ...
 * @return void
 */
function oes_navigation_render_menu_meta_box(): void
{
    ...
    if (!empty($oes->theme_index_pages))
        foreach ($oes->theme_index_pages as $indexPage)
            $menuItems[] = (object)[
                'ID' => ++$objectID,
                'db_id' => 0,
                'menu_item_parent' => 0,
                ...
                'type_label' => 'OES Menu',
                'title' => (($indexPage['label'][$oes->main_language] ?? '[Index]')),
