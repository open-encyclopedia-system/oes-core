 ...
 * Add redirect --------------------------------------------------------------------------------------------------------
 * -------------------------------------------------------------------------------------------------------------------*/
add_action('template_redirect', 'oes_xml_generate_file', 8);

/**
 * Redirect for file generation
 */
function oes_xml_generate_file()
{
 ...
 */
function oes_xml_shortcode($args = [], string $content = null): string
{
    /* get post id */
    global $post;
    return '<span class="' .
 ...
 *
 */
function oes_echo_xml_button(array $args = [])
{
    echo do_shortcode('[oesxmlbutton' . (isset($args['id']) ? ' id="' . $args['id'] . '"' : '') . ']');
}


function oes_export_post_to_xml(array $args = [])
{
    /* check if rdf */
...
function oes_xml_resource(array $args = [], int $level = 0, $rdf = false)
{
...
function oes_xml_toc($oes_post, $args = [])
{
...
function oes_xml_dublin_properties()
{
    return [
        'abstract' => [
