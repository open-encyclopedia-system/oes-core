# Credits

This plugin is developed and maintained by **Maren Welterlich-Strobl**,
Freie Universität Berlin, FUB-IT, Digitale Forschungsinfrastrukturen.
**Roles ([CRediT](https://credit.niso.org/)):** Software, Writing – original draft, Writing – review & editing

Contributor roles below follow the [Contributor Roles Taxonomy (CRediT)](https://credit.niso.org/), a 
standardized vocabulary for
describing individual contributions to a scholarly or software output.

## OES Today

The Open Encyclopedia System is the responsibility of the [Digital Research
Infrastructures (DFI)](https://www.it.fu-berlin.de/die-fub-it/organisation/lsf/digitale-forschungsinfrastruktur/index.html)
division of [FUB-IT](https://www.it.fu-berlin.de/index.html) at [Freie Universität Berlin](https://www.fu-berlin.de/), which also operates and
develops it.

## Predecessor Software

This plugin builds on work carried out under a single, continuous DFG-funded
research project, **"Von 1914-1918-online zu dem Open Encyclopedia System"**
(2011–2020), DFG project number
[195110440](https://gepris.dfg.de/gepris/projekt/195110440?language=en).
The project produced two successive software solutions, described below.
For a narrative account of this history, see
[OES's history](https://www.open-encyclopedia-system.org/en/ueber_uns/historie/index.html) on the official website.

**Antragsteller (grant applicants):** Prof. Dr. Nicolas Apostolopoulos,
Dr. Klaus Ceynowa, Prof. Dr. Oliver Janz, Prof. Dr. Miltos Pechlivanos
**Roles (CRediT):** Funding acquisition, Supervision, Project administration

### Phase 1: 1914-1918-online: International Encyclopedia of the First World War, 2011–2015

The conceptual foundations for OES trace back to the project
[“1914-1918-online: International Encyclopedia of the First World
War.”](https://encyclopedia.1914-1918-online.net/)
A custom-built software solution for this project was developed by CeDiS
and was later replaced, in the follow-up phase, by the standardized OES
solution.

**Prof. Dr. Oliver Janz** — Principal Investigator
**Roles (CRediT):** Conceptualization, Project administration, Supervision, Funding acquisition

**Prof. Dr. Nicolas Apostolopoulos** — Principal Investigator
**Roles (CRediT):** Supervision, Software, Funding acquisition

**Dr. Klaus Ceynowa** — Principal Investigator
**Roles (CRediT):** Supervision, Funding acquisition

**Dr. Jennifer Willenberg** — Project coordination
**Roles (CRediT):** Conceptualization, Project administration

**Ilker Egilmez** — Software development
**Roles (CRediT):** Software

### Phase 2: Open Encyclopedia System (OES), 2016–2020

Building on Phase 1, the **Open Encyclopedia System (OES)** was developed as
a standardized platform for creating, publishing, and maintaining scholarly
online encyclopedias in the humanities and social sciences, implemented as
a WordPress plugin with accompanying themes.

Since October 2020, the OES code has been available under a **GPLv2
license** on [GitHub](https://github.com/open-encyclopedia-system/) for use
and extension by third parties.

**Prof. Dr. Nicolas Apostolopoulos** — Principal Investigator
**Roles (CRediT):** Project administration, Supervision, Funding acquisition

**Prof. Dr. Oliver Janz** — — Principal Investigator
**Roles (CRediT):** Conceptualization, Project administration, Supervision

**Prof. Dr. Miltos Pechlivanos** — Principal Investigator
**Roles (CRediT):** Project administration, Supervision, Funding acquisition

**Dr. Klaus Ceynowa** — Principal Investigator
**Roles (CRediT):** Supervision, Funding acquisition

**Dr. Jennifer Willenberg** — Project coordination
**Roles (CRediT):** Conceptualization, Project administration

**Christoph Schimmel** — Project coordination
**Roles (CRediT):** Project administration, Conceptualization,

**Ilker Egilmez** — Implementation of the OES prototype
**Roles (CRediT):** Software

## Third-Party Code

### Modern Footnotes
This plugin's popup feature is based on **Modern Footnotes** (v1.4.4),
originally developed by **Sean Williams / Prism Tech Studios**
([prismtechstudios.com](http://prismtechstudios.com/)), 2017–2021,
licensed under **GPLv2**.
**Roles (CRediT):** Software (original implementation)

The code was adapted to implement popup functionality (rather than
footnotes). See `includes/popup` for the modified implementation.
Adaptation: **Maren Welterlich-Strobl** — Software (modification)

---

*Main development and authorship of this plugin belongs to Maren Welterlich-Strobl, Freie Universität Berlin,
FUB-IT, Digitale Forschungsinfrastrukturen.
We thank all the individuals and projects named above for the conceptual and
technical foundations this plugin builds upon.*