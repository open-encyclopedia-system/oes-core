...
class libZotero_Http_Response
{
    ...
    {
        error_log("response string $response_str");
...
class Zotero_Library
{
    ...
    public function __construct($libraryType = null, $libraryID = null, $libraryUrlIdentifier = null, $apiKey = null, $baseWebsiteUrl="http://www.zotero.org", $cachettl=0)
    {
        ...
        if($this->libraryType == 'user'){
            $this->libraryBaseWebsiteUrl .= 'users/';
        }
    ...
    public function _request($url, $method="GET", $body=NULL, $headers=array(), $basicauth=array()) {
        ...
        if($this->_cacheResponses && $umethod == 'GET'){
            $cachedResponse = x_apc_fetch($url, $success);
            if($success){
                ...
                $cacheDir = __DIR__."/../zotero/cache/";

                $cacheFilePath = $cacheDir.$urlmd5;

                if (file_exists($cacheFilePath)) {

                    $stat = stat($cacheFilePath);

                    $mtime = $stat['mtime'];

                    $cachedResponse = file_get_contents($cacheFilePath);
                    $cachedResponse =
                        unserialize(base64_decode($cachedResponse));
                    $responseBody = $cachedResponse['responseBody'];
                    $responseInfo = $cachedResponse['responseInfo'];
                    $zresponse =
                        libZotero_Http_Response::fromString($responseBody);
                    $gotCached = true;
...
//            error_log("response body $responseBody $url");
            $zresponse = libZotero_Http_Response::fromString($responseBody);
            ...
            if($this->_cacheResponses && !($zresponse->isError()) ){
                x_apc_store($url, $saveCached, $this->_cachettl);
                $urlmd5 = md5($url);
                $cacheDir = __DIR__."/../zotero/cache/";
                $cacheFilePath = $cacheDir.$urlmd5;
                file_put_contents($cacheFilePath,
                    base64_encode(serialize($saveCached)));
            }
        }
    ...
    public function fetchItems($params = array()){
        $fetchedItems = array();
        $aparams = array_merge(array('target'=>'items', 'format'=>'atom'), array('key'=>$this->_apiKey), $params);
        ...
        $response = $this->_request($reqUrl);
        if($response->isError()){
            throw new Exception("Error fetching items ".$response->getMessage());
        }

        $rawBody = $response->getRawBody();

//        file_put_contents(__DIR__."/db/xml/".md5($reqUrl).".xml",$rawBody);

        $feed = new Zotero_Feed($rawBody);
        $this->_lastFeed = $feed;
        $fetchedItems = $this->items->addItemsFromFeed($feed);
    ...
    public function fetchItemsInJson($keys, $style, $maxcount = 0, $locale = 'en-US'){

        if (empty($keys)) {
            return $keys;
        }

        if (count($keys) > self::FETCH_BATCH_SIZE) {
...
//                error_log("loading $offset/$totalcount");

                $splitkeys = array_splice($keys, 0, self::FETCH_BATCH_SIZE);

                $foundItems =
                    $this->fetchItemsInJson($splitkeys, $style,0,$locale);

                $offset += count($foundItems);

                $items = array_merge($items, $foundItems);

                if (count($foundItems) < self::FETCH_BATCH_SIZE) {
                    break;
                }

                if ($maxcount && $offset >= $maxcount) {
                    break;
                }
        ...
        $params = ['include'=>'bib,data,citation',
        ...
        $aparams = array_merge(array('target'=>'items',
            'format'=>'json'),
            array('key'=>$this->_apiKey), $params);
        ...
        while (true) {
            ...
            try {
                ...
                Oes::error("error fetching zotero (try: $tries): ".$e->getMessage());
                $hasError = true;
            }

            if ($hasError || $response->isError()) {

                if ($tries > 7) {
                    error_log("Error fetching items " . $response->getMessage()." ".$reqUrl);
                    break;
                }
     ...
     * @return array of fetched items
     */
    public function fetchItemVersions($params = array()){
        $fetchedItems = array();
        $aparams = array_merge(array('target'=>'items'), array('key'=>$this->_apiKey), $params);

        $aparams['format'] = 'versions';
